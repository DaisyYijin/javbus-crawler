...
- **精准筛选**：关键词优先级列表（可拖拽排序、显示库内命中数）匹配标签或磁力名，
  推荐磁力或只收命中的影片；磁力优选优先级链可视化（命中数 → 关键词顺序 → 大小/日期决胜 → 列表位置）；
  输入番号实时预览全部磁力排名与将入选的那条（不入库、支持未保存配置）；
...
| `GENRE_CENSORED` / `GENRE_UNCENSORED` | 空 | 按频道分组的类别筛选（目录直接取自站点 /genre 页并按大标题分组，网页点击多选）；留空采集全部类别 |
